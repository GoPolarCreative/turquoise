import { FormEvent, ReactNode, TouchEvent as ReactTouchEvent, useEffect, useMemo, useState } from 'react';
import {
  ArrowRight, Award, Bath, Check, ChevronDown, Clock3, Droplets, Flame,
  Facebook, Gauge, Home as HomeIcon, Instagram, Mail, MapPin, Menu, Phone, ShieldCheck, Sparkles, Star,
  Thermometer, Wrench, X, Zap,
} from 'lucide-react';

function TikTok({ size = 16 }: { size?: number }) { return <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1.04-.1z"/></svg>; }
...
const reviewsUrl = 'https://www.google.com/search?q=turquoise+plumbing&rlz=1C1VDKB_enAU1049AU1049&oq=turquoise+pl&gs_lcrp=EgZjaHJvbWUqBggAECMYJzIGCAAQIxgnMhIIARAuGEMYrwEYxwEYgAQYigUyBggCEEUYOTIGCAMQRRg7MgcIBBAAGIAEMgYIBRBFGDwyBggGEEUYPDIGCAcQRRg80gEIMjU4MmowajeoAgCwAgA&sourceid=chrome&source=chrome.ob&ie=UTF-8#lrd=0x8159013b37d8b44f:0xbf4614194090ec91,1,,,,';
const licenceNumber = '478348C';
...
function trackEvent(action: string, category: string = 'engagement') { if (typeof window !== 'undefined' && (window as { gtag?: (...args: unknown[]) => void }).gtag) { (window as { gtag: (...args: unknown[]) => void }).gtag('event', action, { event_category: category }); } }
const reviews = [
  { name: 'Rachel Bland', quote: locations[0].testimonial, location: 'Shellharbour, NSW' },
  { name: 'Sharon Russel', quote: locations[1].testimonial, location: 'Wollongong, NSW' },
  { name: 'Demi DW', quote: locations[2].testimonial, location: 'Albion Park, NSW' },
];
...
function Header() {
  ...
  return <header className={`site-header ${scrolled ? 'is-scrolled' : ''}`}><div className="shell header-inner">
    ...
    <div className="header-actions"><a className="header-phone" href={phoneHref} onClick={() => trackEvent('click_phone', 'contact')}><Phone size={16} /> {phone}</a><a className="button button-small" href="/contact" onClick={() => trackEvent('click_quote', 'engagement')}>Get a free quote <ArrowRight size={16} /></a></div>
...
function Form({ compact = false }: { compact?: boolean }) {
  ...
  return <form className={`contact-form ${compact ? 'compact' : ''}`} onSubmit={submit}><div className="form-heading"><span className="eyebrow">Start a conversation</span><h2>Tell us what is happening.</h2><p>We will get back to you with a clear next step and no pressure.</p></div><div className="form-grid"><label>Name<input name="name" required placeholder="Your name" /></label><label>Phone<input name="phone" required type="tel" placeholder="0491 241 255" /></label></div><label>Email <span className="form-optional">(optional)</span><input name="email" type="email" placeholder="you@example.com" /></label><label>What do you need help with?<textarea name="message" required rows={compact ? 3 : 4} placeholder="A few details about the job" /></label><button className="button" type="submit">Send enquiry <ArrowRight size={17} /></button><small>By submitting, you agree to be contacted about your enquiry.</small></form>;
}
...
function CTA({ title = 'Need a plumber you can count on?', text = 'Tell us what is happening and we will help you find the right next step.' }) { return <section className="cta-band"><div className="shell cta-inner"><div><span className="eyebrow eyebrow-light">Here when you need us</span><h2>{title}</h2><p>{text}</p></div><div className="cta-actions"><a className="button button-light" href={phoneHref} onClick={() => trackEvent('click_phone', 'contact')}><Phone size={18} /> Call {phone}</a><a className="text-link light" href="/contact" onClick={() => trackEvent('click_quote', 'engagement')}>Send an enquiry <ArrowRight size={16} /></a></div></div></section>; }
function Footer() { return <footer><div className="shell footer-grid"><div className="footer-brand"><a className="brand" href="/"><img src={logo} alt="Turquoise Plumbing logo" /><span className="brand-name">TURQUOISE PLUMBING</span></a><p>Honest, reliable plumbing for homes across the Illawarra. Local care, quality workmanship and no surprises.</p><a className="review-link" href={reviewsUrl} target="_blank" rel="noreferrer"><Star size={15} fill="currentColor" /> Read our Google reviews</a><div className="social-links"><a href="https://www.facebook.com/p/Turquoise-Plumbing-61568082454448/" target="_blank" rel="noreferrer" aria-label="Facebook"><Facebook size={18} /></a><a href="https://www.instagram.com/turquoiseplumbing/" target="_blank" rel="noreferrer" aria-label="Instagram"><Instagram size={18} /></a><a href="https://www.tiktok.com/@turquoiseplumbing" target="_blank" rel="noreferrer" aria-label="TikTok"><TikTok size={18} /></a></div></div><div><h4>Explore</h4><a href="/about">About us</a><a href="/service-areas">Service areas</a><a href="/contact">Contact</a><a href="/#faq">FAQs</a></div><div><h4>Popular services</h4>{services.slice(0, 5).map(service => <a key={service.slug} href={`/services/${service.slug}`}>{service.name}</a>)}</div><div><h4>Contact</h4><a href={phoneHref}><Phone size={14} /> {phone}</a><a href={`mailto:${email}`}><Mail size={14} /> {email}</a><span><MapPin size={14} /> Barrack Heights, NSW</span><span><Clock3 size={14} /> 24/7 emergency service</span><span><ShieldCheck size={14} /> Licence No. {licenceNumber}</span></div></div><div className="shell footer-bottom"><span>© 2026 Turquoise Plumbing Pty Ltd. All rights reserved.</span><span>Licence No. {licenceNumber}</span><a href="https://www.itscold.com.au" target="_blank" rel="noreferrer">Website by Go Polar</a></div></footer>; }
function Layout({ children }: { children: ReactNode }) { return <><Header /><main>{children}</main><Footer /><a className="mobile-call" href={phoneHref} onClick={() => trackEvent('click_phone', 'contact')}><Phone size={18} /> Emergency call</a></>; }

function Hero() { return <section className="hero"><div className="hero-glow" /><div className="shell hero-grid"><Reveal className="hero-copy"><img className="hero-brand-mark" src={logo} alt="" /><span className="eyebrow">Local plumbers. Serious about doing it right.</span><h1>Water, gas and plumbing handled with <em>care.</em></h1><p>Turquoise Plumbing is your family-owned local team for honest advice, quality workmanship and reliable help across the Illawarra.</p><div className="hero-actions"><a className="button" href={phoneHref} onClick={() => trackEvent('click_phone', 'contact')}><Phone size={17} /> Call {phone}</a><a className="text-link" href="#services">Explore services <ArrowRight size={16} /></a></div><div className="trust-mini"><span><ShieldCheck size={17} /> Licensed & insured</span><span><Star size={17} fill="currentColor" /> 60+ Google reviews</span><span><Clock3 size={17} /> Available 24/7</span></div></Reveal><Reveal className="hero-form"><Form /></Reveal></div></section>; }
...
function Testimonials() {
  ...
  const go = (dir: number) => setActive(prev => (prev + dir + reviews.length) % reviews.length);
  const onTouchStart = (e: ReactTouchEvent<HTMLDivElement>) => setTouchStart(e.touches[0].clientX);
  const onTouchEnd = (e: ReactTouchEvent<HTMLDivElement>) => { if (touchStart === null) return; const diff = touchStart - e.changedTouches[0].clientX; if (Math.abs(diff) > 40) go(diff > 0 ? 1 : -1); setTouchStart(null); };
  return <section className="section testimonials-section"><div className="shell"><div className="section-heading centered"><span className="eyebrow">Kind words from local homes</span><h2>Good work gets <em>noticed.</em></h2></div>
    <div className="review-carousel" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      <div className="review-track" style={{ transform: `translateX(-${active * 100}%)` }}>
        {reviews.map((review) => <div className="review-slide" key={review.name}><div className="review-card"><div className="stars">★★★★★</div><p>{review.quote}</p><div className="testimonial-meta"><div className="avatar">{review.name[0]}</div><span><b>{review.name}</b><small>{review.location}</small></span></div></div></div>)}
      </div>
      <div className="review-nav">
        <button onClick={() => go(-1)} aria-label="Previous review"><ChevronDown size={20} className="review-nav-prev" /></button>
        <div className="review-dots">{reviews.map((_, i) => <button key={i} className={`review-dot ${i === active ? 'active' : ''}`} onClick={() => setActive(i)} aria-label={`Go to review ${i + 1}`} />)}</div>
        <button onClick={() => go(1)} aria-label="Next review"><ChevronDown size={20} className="review-nav-next" /></button>
      </div>
    </div>
    <div className="center-link"><a className="text-link" href={reviewsUrl} target="_blank" rel="noreferrer">Read our Google reviews <ArrowRight size={16} /></a></div>
  </div></section>;
}
